...
mod tests {
    ...
    fn test_skill_deep_only_tactic_gate() {
        ...
        let result = MultiPVResult {
            lines: vec![
                PVLine { mv: deep_mv, score: 200, depth: 5, pv: vec![deep_mv] },
                PVLine { mv: plain_mv, score: 200, depth: 5, pv: vec![plain_mv] },
            ],
            stats: SearchStats { nodes: 0, tt_capacity: 0, tt_used: 0, tt_fill_permille: 0 },
            shallow_best_changed: false,
        ...
        assert!(never_deep, "deep-only tactic must not surface when disallowed");
        ...
        assert!(sometimes_deep, "deep-only tactic must be reachable when allowed");
    }
    ...
    fn test_deep_tactic_reference_depth_scales_with_budget() {
        ...
        for config in SKILL_CONFIGS {
            let Some(cap) = config.depth_cap else { continue };
            ...
            assert!(reference >= prev, "reference depth must not shrink up the ladder");
            prev = reference;
        }
